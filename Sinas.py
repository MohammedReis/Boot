...
print('\nQuantos dias para catalogar: ', end='')
dias = int(input())
...
for par in catalogacao:
    for horario in sorted(catalogacao[par]):
        ...
        if ok == True:
            ...
            print(msg)
            open('sinaisDigitais' + (datetime.now()).strftime('%Y-%m-%d') + ' _ ' + str(timeframe) +'M.txt', 'a').write(horario + ',' + par + ',' + catalogacao[par][horario]['dir'].strip() + ',' + str(catalogacao[par][horario]['%'])+'%'+ '\n')
